...
// Birinchi ish
const moviesEl = document.getElementById('movieAll')

//Ikkinchi ish

function createMovieDiv(ObjectDiv){
    let divEl = document.createElement('div')
    divEl.className = 'movie'

  divEl.innerHTML = `
    <img src="img/${ObjectDiv.poster}" class="movie__poster">
    <div class="movie__info">
        <h2 class="movie__title">${ObjectDiv.title}</h2>
        <h3 class="movie__genre">${ObjectDiv.genres}</h3>
        <button class="movie__btn" type="button" data-id="${ObjectDiv.id}">Add to watchlist</button>
    </div>
    `;
    return divEl
}
...
function filterByCategory(genre){
    ...
    let result = movies.filter(function(el){
        return el.genres.includes(genre)
    });
    return result
}

// let comedymovies = filterByCategory('Comedy');
    ...
    sidebarEl.addEventListener('click', function(event){
        if(event.target.className == 'genre__item'){
            ...
            if(selectedGenre != "All"){
                 selectedMovies = filterByCategory(selectedGenre);
            } else{
                selectedMovies = movies
            }
            moviesEl.innerHTML = ''
            appendMovieCategory(selectedMovies)
        }
    })
}
...
function appendMovieCategory(Movies){
    Movies.forEach((el) => {
        let DivMovie = createMovieDiv(el)
        moviesEl.appendChild(DivMovie)
    })
}
...
function searchMovie(searchTitle){
    let resultArr = movies.filter(function(el){
        return el.title.toLowerCase().includes(searchTitle)
    });
    return resultArr
}
...
searchInput.addEventListener('keyup', function(){
    let searchedMovie = searchMovie(searchInput.value)
    
    moviesEl.innerHTML = '';
    appendMovieCategory(searchedMovie)
})

// To`qqizinchi ish

let myFavs = []
function addFilmToMyFavs(id){
...
if(moviesEl){
    moviesEl.addEventListener('click', function(event){
        if(event.target.className == 'movie__btn'){
            ...
            addFilmToMyFavs(idOfCurrentFilm)
            cartItemsToLocal()
        }
    })
...
//o`n birinchi ish
function cartItemsToLocal(){
    localStorage.setItem('myMovies', JSON.stringify(myFavs))
}


// O`n ikkinchi ish
function cartItemsFromLocal(){
    let result =  JSON.parse(localStorage.getItem('myMovies'))
    return result ? result : []
 }

 // O`n uchinchi ish

 function createMyMoviesCart(ObjectDiv){
    let divEl = document.createElement('div')
    divEl.className = 'movie'
  ...
  divEl.innerHTML = `
    ...
    <div class="movie__info">
        <h2 class="movie__title">${ObjectDiv.title}</h2>
        <h3 class="movie__genre">${ObjectDiv.genres}</h3>
        <button class="movie__btn" type="button" data-id="${ObjectDiv.id}">Remove</button>
    </div>
    `;
    return divEl
 }
...
const filmCartEl = document.getElementById('filmcart')
 if(filmCartEl){
...
function showMovies(){
    filmCartEl.innerHTML = ''
    let mycart = cartItemsFromLocal()
    mycart.forEach(function(elid){
        console.log(elid)
        let films = movies.filter(function(item){
            return item.id == elid
        })
        
        let element = createMyMoviesCart(films[0])
        filmCartEl.appendChild(element)
    })
}

//O`n beshinchi ish 
if(filmCartEl)
filmCartEl.addEventListener('click', function(event){
    if(event.target.className == 'movie__btn'){
        let delateid = event.target.getAttribute('data-id')
        let mycart = cartItemsFromLocal() 
        mycart.splice(mycart.indexOf(+delateid),1)
        localStorage.setItem('myMovies', JSON.stringify(mycart))
        showMovies()
    }
})